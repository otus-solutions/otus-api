...
public class DefaultTubeGeneratorTest {	
	...
	public void method_should_getTubeDefinitions(){
        ...
        String expectTypeTubeGel = tubeDefinitions.stream().filter(t->t.getType().equals("GEL")).findFirst().get().getType();
        assertEquals(expectTypeTubeGel, defaultTubeGenerator.getTubeDefinitions(tubeSeed).get(0).getType());
        
        String expectTypeTubeFloride = tubeDefinitions.stream().filter(t->t.getType().equals("FLORIDE")).findFirst().get().getType();
