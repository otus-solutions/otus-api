...
public class TubeConfigurationTest {
	...
	public void setup() {	
		...
		tubeConfiguration = new Gson().fromJson(json, LaboratoryConfiguration.class);
	...
	@Test
	public void should_deserialize_for_codeDefinitions() {
		CodeDefinition codeDef = tubeConfiguration.getCodeDefinitions();
		assertThat(codeDef.getTubeCode(), equalTo(1) );
	}

	@Test
	public void should_deserialize_for_groupDefinitions() {
		GroupDefinition groupDef = tubeConfiguration.getGroupDefinitions();
		assertThat(groupDef.getDefaultGroup().get(0).getName(), equalTo("regular") );
	}
